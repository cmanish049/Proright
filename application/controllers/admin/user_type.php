...
class User_type extends Admin_Controller
{
    public function __construct()
    {
        ...
        $this->load->model(array('user_type_model'));

        $this->data['module'] = $this->auth->get_module($module);
        $this->data['page_title'] = $this->data['module']['module_plural_label'];
        $this->data['index_url'] = admin_url('user_type/index/');
        $this->data['limit'] = 10;
    }

    public function index()
    {
        $this->data['edit_url'] = admin_url("user_type/edit/window/modal");
        $this->data['page_title'] = $this->data['module']['module_plural_label'];

        $this->template->view_parts('content', 'user_type/index_view', $this->data)
    ...
    public function grid()
    {
        ...
        try
        {
            ...
            $this->user_type_model->set_extra_from_url($extra);

            $json['rows'] = $this->user_type_model->get_rows($extra);

            unset($extra['limit'], $extra['offset']);
            $json['total'] = $this->user_type_model->get_count($extra);
        }
    ...
    public function delete()
    {
        ...
        {
            $this->db->trans_start();
            $this->user_type_model->delete($this->id);
    ...
    public function edit()
    {
        ...
        if($this->_form_validation() === TRUE)
        {
            ...
            try
            {
                ...
                if(empty($this->id))
                {
                    $this->id = $this->user_type_model->insert($db_data);
                    $this->uri_assoc['id'] = $this->id;
                }
                else
                {
                    $this->user_type_model->update($this->id, $db_data);
                }
                ...
                {
                    js_close_modal('user_typeModal');
                }
                elseif(is_ajax())
                {
                    $row = $this->user_type_model->get_row_by_id($this->id);
                    $json['row'] = $row;
                    $json['item'] = array(
                        'id' => $row->user_type_id,
                        'value' => $row->user_type_id,
                        'text' => $row->user_type_name,
                    );
                    $this->output->_display($this->fastjson->encode($json));
                    exit;
                }
        ...
        if(is_ajax())
        {
            ...
            $this->template->view_parts('content', 'user_type/form_view', $this->data);
        }
        else
        {
