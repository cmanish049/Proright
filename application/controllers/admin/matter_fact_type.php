...
class Matter_fact_type extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $module = 'matter_fact_type';
        $this->auth->is_authorized($module);
        $this->load->model(array('matter_fact_type_model'));
        
        $this->data['module'] = $this->auth->get_module($module);
        $this->data['page_title'] = $this->data['module']['module_plural_label'];
        $this->data['index_url'] = admin_url('matter_fact_type/index/');        
        $this->data['limit'] = 10;
    }

    public function index()
    {
        $this->data['edit_url'] = admin_url("matter_fact_type/edit/window/modal");
        $this->data['page_title'] = $this->data['module']['module_plural_label'];

        $this->template->view_parts('content', 'matter_fact_type/index_view', $this->data)
    ...
    public function grid()
    {
        ...
        try 
        {
            ...
            $this->matter_fact_type_model->set_extra_from_url($extra);        

            $json['rows'] = $this->matter_fact_type_model->get_rows($extra);

            unset($extra['limit'],$extra['offset']);
            $json['total'] = $this->matter_fact_type_model->get_count($extra);
        }
    ...
    public function delete()
    {
        ...
        {
            $this->db->trans_start();
            $this->matter_fact_type_model->delete($this->id);
    ...
    */
    ...
    public function edit()
    {
        ...
        {
            $db_data = array(
                'fact_type_name' => $this->input->post('fact_type_name', TRUE),
'is_active' => $this->input->post('is_active', TRUE),
            ...
            {       
                ...
                if(empty($this->id))
                {
                    $this->id = $this->matter_fact_type_model->insert($db_data);
                    $this->uri_assoc['id'] = $this->id;
                }
                else
                {
                    $this->matter_fact_type_model->update($this->id, $db_data);
                }
        ...
        $this->data['row'] = $this->matter_fact_type_model->get_row_by_id($this->id, array());

        
        $this->template->view_parts('content', 'matter_fact_type/form_view', $this->data)
                ->title($this->data['page_title'])
                ->build();
    }
