...
class Location extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['index_url'] = admin_url('location/index/');        
        $this->data['limit'] = 10;
    }

    public function index()
    {
        $this->data['edit_url'] = admin_url("location/edit/window/modal");
        $this->data['page_title'] = $this->data['module']['module_plural_label'];

        $this->template->view_parts('content', 'location/index_view', $this->data)
    ...
    public function grid()
    {
        ...
        try 
        {
            ...
            $this->location_model->set_extra_from_url($extra);        

            $json['rows'] = $this->location_model->get_rows($extra);

            unset($extra['limit'],$extra['offset']);
            $json['total'] = $this->location_model->get_count($extra);
        }
    ...
    public function delete()
    {
        ...
        {
            $this->db->trans_start();
            $this->location_model->delete($this->id);
    ...
    private function _upload_image($id='', $image_name='')
    {
        ...
        if($this->file->upload_file('image', $image_name))
        {
            ...
            $this->location_model->update($id, array('image_id' => $image_id));
        }
        else
        {
    ...
    public function edit()
    {
        ...
        if($this->_form_validation() === TRUE)
        {
            ...
            {       
                ...
                if(empty($this->id))
                {
                    $this->id = $this->location_model->insert($db_data);
                    $this->uri_assoc['id'] = $this->id;
                }
                else
                {
                    $this->location_model->update($this->id, $db_data);
                }
        ...
        $this->data['row'] = $this->location_model->get_row_by_id($this->id, array());

        
        $this->template->view_parts('content', 'location/form_view', $this->data)
                ->title($this->data['page_title'])
                ->build();
    }
